 ...
 *
 * Note: Ordered lists are intentionally not recognized. Only unordered lists
 * starting with '-', '*', or '+' (optionally preceded by up to 3 spaces) are
 * parsed as list blocks. This constraint is to restrict LLM output formatting.
 */
import { getCiteMarkerRegex, parseCitationPayload, type CitationTarget } from "./cite";
...
export type InlineToken = TextSegment | InlineCodeSegment | MathSegment | CiteToken;
...
/**
 * Parse a Markdown-ish string with math support into lightweight blocks.
 *
 * Constraints:
 * - Only unordered lists ("-", "*", "+") are recognized; ordered lists are disabled.
 * - No nested list levels are produced; lists are flat sequences of items.
 */
...
function tryParsePipeTable(
  ...
  const headerTokens: InlineToken[][] = headerCells.map((text, i) =>
    parseInline(
      text.trim(),
      headerLine.startIndex + headerLine.line.indexOf(text),
      mathSegments,
      text.length,
      { enableCite: false },
    ),
...
function consumeHeading(
  ...
  const tokens = parseInline(
    text,
    lineInfo.startIndex + lineInfo.line.indexOf(text),
    mathSegments,
    text.length,
    { enableCite: false },
  );
...
function matchList(line: string): { ordered: boolean; marker: string } | null {
  // Only recognize unordered lists; ordered lists intentionally disabled.
...
function consumeList(
  ...
  while (true) {
    ...
    const tokens = parseInline(
      content,
      line.startIndex + line.line.indexOf(content),
      mathSegments,
      content.length,
      { enableCite: true },
    );
...
interface ParseInlineOptions { enableCite?: boolean }
...
function parseInline(
  ...
  while (index < length) {
    ...
    // [-[cite: ...]-] detection – only when enabled for the current inline context
    if (options.enableCite) {
      const slice = text.slice(index);
      const re = getCiteMarkerRegex();
      const m = re.exec(slice);
      if (m && (m.index ?? 0) === 0) {
        const payload = m[1] || "";
        const parsed = parseCitationPayload(payload);
        const matchEnd = index + (m[0]?.length || 0);
        ...
        // fallthrough if parsing fails: treat as normal text
      }
    }
